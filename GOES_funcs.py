...
def process_files(filelist, year, day, limits, GLM, borders=False, enhance=False):
    ...
    for i, file in enumerate(filelist):
        ...
        date = process_ABI(fs.open(file, 'r'), fig, ax,
                           projection, borders, enhance)
        ...
        if GLM:
            ...
            syear, sday, shour, smin, ssec, \
                ...
                cyear, cday, chour, cmin, csec = np.array(
                    match[0], dtype=np.int32)
...
def process_ABI(file, fig, ax, projection, borders, enhance):
    ...
    date = datetime.datetime(2000, 1, 1, 12) + \
        datetime.timedelta(seconds=float(timeoff))
    print("Processing %02d:%02d:%02.3f..." %
          (date.hour, date.minute, date.second), end='')
    ...
    if enhance:
        for ci in range(3):
            color_corr[:, :, ci] = exposure.equalize_adapthist(
                color_corr[:, :, ci], clip_limit=0.015)
    else:
    ...
    RGB_IR = np.dstack([np.maximum(R, cleanIR), np.maximum(
        G, cleanIR), np.maximum(B, cleanIR)])
    ...
    ax.imshow(RGB_IR, origin='upper', extent=(x.min(), x.max(),
              y.min(), y.max()), transform=geos, interpolation='hermite')
...
def get_GLM(start_date, end_date, fs):
    ...
    slist = fs.ls('noaa-goes16/GLM-L2-LCFA/%d/%03d/%02d/' %
                  (syear, sday, shour))
    elist = fs.ls('noaa-goes16/GLM-L2-LCFA/%d/%03d/%02d/' %
                  (eyear, eday, ehour))
...
def plot_GLM(GLM_list, fs, fig, ax, start_date, end_date):
    for GLMfile in GLM_list:
        ...
        event_date = [
            date + datetime.timedelta(seconds=float(ti)) for ti in event_time]
        mask = np.array([(tt >= start_date) & (tt <= end_date)
                        for tt in event_date], dtype=np.bool)
        ...
        ax.scatter(event_lon, event_lat, s=2, c=event_energy, vmin=-15,
                   vmax=-10, cmap='Reds_r', transform=ccrs.PlateCarree())
